    ...
    return (
        <div onClick={() => { setIsDropdown2Open(false); setIsDropdownOpen(false) }} className='bg-[#080708] min-h-screen pt-10 w-full'>
            ...
            <div className="main px-6 md:px-10">
                ...
                <div className="searchbar my-6 flex flex-col md:flex-row items-center justify-between ">
                    <form onSubmit={handleSearchSubmit} className='relative w-full sm:px-1 md:w-max'>
                        <input
                            ...
                            type="search"
                            id='search'
                            className='border border-gray-500 active:border-cyan-400 focus:border-cyan-400 focus:shadow-lg shadow w-full md:w-[400px] h-[45px] rounded-xl p-4 active:outline-none outline outline-none placeholder:text-gray-500 text-white'
                    ...
                    <div className="flex w-full justify-between md:justify-end items-center my-2 md:my-0">
                        ...
                        >
                            ...
                            <div className={`absolute top-12 w-[200px] h-max bg-[#080708] rounded-2xl py-3 border border-cyan-400 shadow-lg left-0 transform-3d origin-top ${isDropdown2Open ? 'opacity-100 z-[120]' : 'opacity-0 -z-[20] scale-y-[0]'} transition-all duration-300 ease-in-out`}>
                                {[
                                    { name: "Men's Choice", value: "Men's Choice" },
                                    { name: "Women's Choice", value: "Women's Choice" },
                                    { name: "Black", value: "Black" },
                                    { name: "Silver", value: "Silver" },
                                    { name: "Gold", value: "Gold" }
                                ].map((item, index) => (
                                    <div key={index} onClick={(e) => handleCategorySelect(item.value)} className="flex justify-start px-4 items-center w-full py-2 hover:bg-[#fefefe1f]">
                                        {item.name}
                                    </div>
                                ))}
                            </div>
                        ...
                        >
                            ...
                            <div className={`absolute top-12 w-[200px] h-max bg-[#080708] rounded-2xl py-3 border border-cyan-400 shadow-lg right-0 transform-3d origin-top ${isDropdownOpen ? 'opacity-100 z-[120]' : 'opacity-0 -z-[20] scale-y-[0]'} transition-all duration-300 ease-in-out`}>
                                <div onClick={(e) => handleSortFilterSelect('New to Old')} className="flex justify-start px-4 items-center w-full py-2 hover:bg-[#fefefe1f]">
                                    New to Old
                                </div>
                                <div onClick={(e) => handleSortFilterSelect('Old to New')} className="flex justify-start px-4 items-center w-full py-2 hover:bg-[#fefefe1f]"> {/* Added Old to New */}
                                    Old to New
                                </div>
                                <div onClick={(e) => handleSortFilterSelect('Price: High to Low')} className="flex justify-start px-4 items-center w-full py-2 hover:bg-[#fefefe1f]">
                                    Price: High to Low
                                </div>
                                <div onClick={(e) => handleSortFilterSelect('Price: Low to High')} className="flex justify-start px-4 items-center w-full py-2 hover:bg-[#fefefe1f]"> {/* Added Low to High */}
                                    Price: Low to High
                                </div>
                            </div>
                        </button>
